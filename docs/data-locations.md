...
> credential store. Milestone 7 also wires
> `settings.json` and `secrets.enc` to real IPC channels for the first time:
> `settings:get`/`settings:update` and
> `secrets:status`/`secrets:write`/`secrets:clear`, all through the
> unmodified Milestone 5 permission pipeline. `permissions\policy.json` and
> `state\emergency.json` are still read fresh on every one of those calls but
> still have no writer or channel of their own reachable from the running
> application. No directory or file is created merely by reading; a directory
> is created lazily, only on the first write.
