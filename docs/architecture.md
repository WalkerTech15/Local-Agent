...
> have **zero diff** for the seventh consecutive milestone. Phase 2 Milestone 9
> adds the workflow engine (`main/workflow-store.ts`,
> `main/workflow-runner.ts`, over the pure `shared/workflow/`) — nine more
> channels, one advisory progress event and three more action types, and once
> again not one new capability: a workflow is a saved recipe for running an
> agent profile that already exists, every step it may name is a Milestone 7
> agent tool, and its steps are executed by the _same_ step runner an agent run
> uses. A workflow sits at the narrow end of
> `workflow ⊆ agent profile ⊆ permission policy` and cannot start itself: its
> trigger is an enum with one member. The permission engine, executor, pipeline
> and audit writer have **zero diff** for the eighth consecutive milestone.
> Phase 2 Milestone 10 adds Windows automation (`main/windows-automation.ts`,
> over the pure `shared/automation/`) — three more channels and two more
> action types, both routed through the unmodified `runAction` /
> `handleActionProposal` pipeline. Every one of the fifteen registered tools —
> launching an approved application, opening an approved folder or website,
> focusing this application's own window, running a registered script — is a
...
| `main/workflow-store` **(exists)**      | Loads, validates and atomically writes workflow definitions. Fails closed to an empty store. Refuses to edit or delete a workflow that is running. | Merge a partially-valid document; let a definition name a non-manual trigger; store a credential.                           |
| `main/workflow-runner` **(exists)**     | Drives one manual run: asks the pure decider what to do next, calls the shared agent step executor, classifies the result.                         | Decide that anything is permitted; call a model; execute a step itself; start without a person.                             |
| `main/windows-automation` **(exists)**  | Performs exactly one registered tool: resolves it, spawns or asks the OS shell, verifies a concrete success signal.                                | Accept a path, a URL or a command from a request; run through a shell; kill an already-launched process; schedule anything. |
